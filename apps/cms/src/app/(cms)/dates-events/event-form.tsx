...
export function DataForm({ mode = "create", initialData, id }: DataFormProps) {
  ...
  const onSubmit = async (data: Event) => {
    ...
    if (result?.success) {
      toast({
        title:
          mode === "create" ? "Successfully created." : "Succesfully updated",
      });
      setOpen(false);
    } else {
      toast({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
        ...
        {mode === "update" && (
          <DialogDescription>Editing event ID: {id}</DialogDescription>
        )}
        <Form {...form}>
...
const ErrorTabsTrigger = ({
  ...
  return (
    <TabsTrigger
      value={value}
      className={hasError ? "font-medium text-red-500" : ""}
    >
      {children}
    </TabsTrigger>
  );
